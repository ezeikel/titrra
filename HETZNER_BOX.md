...
- [`ezeikel/bump-circle`](https://github.com/ezeikel/bump-circle)
- [`ezeikel/parking-ticket-pal`](https://github.com/ezeikel/parking-ticket-pal)
...
- [`ezeikel/genwel`](https://github.com/ezeikel/genwel)
...
- [`ezeikel/salt-mammal`](https://github.com/ezeikel/salt-mammal)
- [`ezeikel/titrra`](https://github.com/ezeikel/titrra) → this repo
...
| **Genwel banking worker**            | `ezeikel/genwel`                    | `/opt/genwel`          | (pending deploy) | `genwel-worker.service`                         | **3080** | Hono HTTP server for the Genwel budgeting app. Runs the long, rate-limited bank work that Vercel's serverless ceiling can't finish: `POST /sync/transactions` (bearer-auth) fetches TrueLayer accounts/cards/transactions and drains the whole AI-categorization backlog (gpt-5.6-luna) to completion — replacing a Next.js `after()` that only did ~100 txns/call. Monorepo worker at `apps/worker/`; imports `@genwel/banking` + `@genwel/db` (workspace:*). Runs under Bun; no Playwright/xvfb. Triggered by the web app on bank-connect + dashboard sync. |
| **Parking Ticket Pal blog worker (monorepo)** | `ezeikel/parking-ticket-pal`   | `/opt/parking-ticket-pal` | (pending deploy) | `parking-ticket-pal-worker.service`             | **3090** | Hono HTTP server for the Parking Ticket Pal monorepo — the FIRST worker in that repo, distinct from the standalone scraper repo at `/opt/worker` (port 3002). AI blog generation for the UK parking/PCN-education SEO engine — `POST /generate/blog` (bearer-auth) drafts a PCN-education post into Sanity (project `db0afjdo`) via Claude, with a featured image (Pexels + Opus-4.8 judge + gpt-image-2), auto-published. Monorepo worker at `apps/worker/`; imports `@parking-ticket-pal/db` + `@parking-ticket-pal/types` (workspace:*). Runs under Bun; no Playwright/xvfb. Triggered by the web app's daily Vercel cron. |
| **Bump Circle blog worker**          | `ezeikel/bump-circle`               | `/opt/bump-circle`     | (pending deploy) | `bump-circle-worker.service`                    | **3100** | Hono HTTP server for the Bump Circle pregnancy/parenthood app. AI blog generation — `POST /generate/blog` (bearer-auth) drafts a source-grounded pregnancy/parenthood post into Sanity (project `l7eqko6r`), auto-published. Monorepo worker at `apps/worker/`; imports `@bump-circle/db` + `@bump-circle/types` + `@bump-circle/sanity-client` (workspace:*). Runs under Bun; no Playwright/xvfb. Triggered by the web app's daily Vercel cron. |
| **Salt Mammal blog worker**          | `ezeikel/salt-mammal`               | `/opt/salt-mammal`     | (pending deploy) | `salt-mammal-worker.service`                    | **3110** | Hono HTTP server for the Salt Mammal recovery-companion app. AI blog generation for the recovery-education SEO engine — `POST /generate/blog` (bearer-auth) drafts a no-shame recovery post into Sanity via Claude, with a featured image (Pexels + Opus-4.8 judge + gpt-image-2), auto-published. Monorepo worker at `apps/worker/`; imports `@salt-mammal/db` + `@salt-mammal/types` (workspace:*). Runs under Bun; no Playwright/xvfb. Triggered by the web app's daily Vercel cron. |
...
> **`/opt/parking-ticket-pal/` is NO LONGER stale.** It used to be a leftover clone of the PTP _monorepo_ (`ezeikel/parking-ticket-pal`) from an abandoned Jan 2026 attempt to run PTP on the box directly. As of July 2026 it is an **active inhabitant**: the monorepo's first worker (`parking-ticket-pal-worker.service`, port **3090**, AI blog generation into Sanity) lives at `/opt/parking-ticket-pal/apps/worker/`. See the [Current inhabitants](#current-inhabitants) table. **Two PTP repos, two directories — keep them straight:** `/opt/worker` = the standalone scraper repo (`ezeikel/parking-ticket-pal-worker`, port 3002, scrapers + automation API); `/opt/parking-ticket-pal` = the monorepo (`ezeikel/parking-ticket-pal`, port 3090, blog worker). See [gotcha #9](#known-gotchas).
...
| **3080**    | **GENWEL `genwel-worker`**                                                    |
| **3090**    | **PTP-monorepo `parking-ticket-pal-worker`**                                  |
| **3100**    | **BUMPCIRCLE `bump-circle-worker`**                                           |
| **3110**    | **SALTMAMMAL `salt-mammal-worker`**                                           |
...
9. **Two PTP repos, two worker directories — BOTH now live.** `/opt/worker/` is `ezeikel/parking-ticket-pal-worker` — the standalone worker repo with the Hono server, scrapers, and automation API (port **3002**, services `worker-api.service` + `worker-watchdog.service`). `/opt/parking-ticket-pal/` is `ezeikel/parking-ticket-pal` — the monorepo with the web/mobile app, which as of July 2026 ALSO runs a worker: its first, the AI blog generator at `apps/worker/` (port **3090**, service `parking-ticket-pal-worker.service`). They sound similar and share the "parking-ticket-pal" name but are **completely different repos, directories, ports, and systemd units** — the scraper/automation worker (`/opt/worker`, 3002) vs the monorepo blog worker (`/opt/parking-ticket-pal`, 3090). **Watch the systemd unit name collision:** the monorepo's unit is `parking-ticket-pal-worker.service` while the scraper repo's is `worker-api.service` — do NOT `systemctl restart parking-ticket-pal-worker` expecting the scraper. Don't confuse them when debugging.

---

## Changelog

- **2026-07-13** — Fleet blog-generation-on-box migration: three AI blog-generation workers added to the runbook, each `POST /generate/blog` drafting into Sanity and auto-publishing off a daily Vercel cron. **Salt Mammal** `salt-mammal-worker` (recovery-education content) → port **3110**, box dir `/opt/salt-mammal`. **Bump Circle** `bump-circle-worker` (pregnancy/parenthood content, Sanity project `l7eqko6r`) → port **3100**, box dir `/opt/bump-circle`. **Parking Ticket Pal monorepo** `parking-ticket-pal-worker` (UK parking/PCN-education content, Sanity project `db0afjdo`) → port **3090**, box dir `/opt/parking-ticket-pal` — the monorepo's FIRST worker, distinct from the standalone scraper repo at `/opt/worker` (3002); `/opt/parking-ticket-pal` is now an active inhabitant, no longer a stale clone (see gotcha #9). All three: in-repo systemd units + `deploy-*-worker.yml` authored, code merged to main, box not yet provisioned (pending SSH first-time setup).
...
- **2026-07-11** — Genwel added to the runbook: `genwel-worker` (TrueLayer transaction sync + AI categorization, draining the whole backlog off Vercel's serverless timeout) allocated port **3080**, box dir `/opt/genwel`. In-repo systemd unit + `deploy-genwel-worker.yml` authored; box not yet provisioned (pending SSH first-time setup).
