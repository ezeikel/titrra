...
// Bump the Gradle daemon heap so KSP (:expo-updates:kspReleaseKotlin) and
// AndroidLint (:expo-modules-core:lintVitalAnalyzeRelease) don't OOM. The
// Expo-generated android/gradle.properties ships
// `org.gradle.jvmargs=-Xmx2048m -XX:MaxMetaspaceSize=512m`, and 512 MiB of
// metaspace isn't enough for our native-dep set (Skia, Expo modules, Sentry,
// RevenueCat, FB SDK, Google Sign-In, wgpu, ...) — the release build dies with
// `OutOfMemoryError: Metaspace`. android/ is regenerated by `expo prebuild
// --clean`, so this can't be a hand-edit there; it has to be a config plugin
// that re-applies on every prebuild. (Same fix as chunky-crayon / salt-mammal.)
